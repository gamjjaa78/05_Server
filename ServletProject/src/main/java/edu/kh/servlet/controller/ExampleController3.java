...
@WebServlet("/coffee") //반드시 소괄호열어 맵핑주소 직접 기재, 자동완성안됨
public class ExampleController3 extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String orderer=req.getParameter("orderer");
		String coffee=req.getParameter("coffee");
		String type=req.getParameter("type"); //radio, 택일, 아이스냐 핫이냐
		
		String option=req.getParameter("opt");
		String [] optionArr=req.getParameterValues("opt");
		//req.getParameter("name속성값") 같은 name 속성가진 여러개 값 중 첫번째 값만 반환
		//req.getParameterValues("name속성값") 같은 name 속성가진 모든 값을 배열(String[])로 반환
		
		System.out.println(orderer);
		System.out.println(coffee);
		System.out.println(type);
		System.out.println(option); //둘 다 선택시 shot
