...
//클라이언트로부터 "/example1" 이라는 요청받아 처리한 후
//응답을 해줄 서블릿클래스 ExampleController1을 만든다.
...
public class ExampleController1 extends HttpServlet{
	...
	//doGet() 메서드
	//GET 방식 요청을 처리하는 메서드
	//HttpServlet을 상속받으면 사용할 수 있는 메서드, 오버라이딩으로 사용
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//HttpServletRequest*********
		//클라이언트 요청시 생성되는 객체
		//요청시 전달된 데이터, 요청한 클라이언트 정보, 유연한 요청처리위한 객체제공
				
		//HttpServletResponse*********
		//클라리언트 요청시 생성되는 객체
		//서버가 클라이언트에게 응답하기 위한 방법제공
		//서버가 클라이언트 쪽으로 만들어진 페이지를 제공하기 위해
		//출력용 스트림 제공(PrintWriter):
		
		System.out.println("---이름, 나이를 입력받아 처리하는 서블릿 코드 ---");
		
		//요청시 입력된 이름, 나이 전달받아오기
		//Parameter 매개변수==다른곳의 값을 전달받아올 때 사용
		//req.getParameter("name속성값");
		//->요청시 전달된 데이터 중 name 속성값이 일치하는 데이터의 value를 얻어와
		//String 형태로 반환**********
		//HTML에서 얻어오는 모든 값은 String 형!!!!!!!!!!!
		String name = req.getParameter("inputName"); //****중요
		String age = req.getParameter("inputAge"); //****중요
		...
		//1.서버에서 HTML 코드만들어 클라이언트에게 전달==응답(response)
		//2.응답받은 내용을 클라이언트의 브라우저가 해석하여 화면에 출력해줌(렌더링)
		...
		//응답처리4 : 출력스트림 이용해 클라이언트에게 HTML 코드제공(응답)
