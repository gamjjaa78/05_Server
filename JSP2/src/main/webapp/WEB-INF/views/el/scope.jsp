...
<body>
	...
	<pre>
		Servlet/JSP에는 4종류 범위를 나타낸 내장객체존재
		->각 종류마다 영향끼치는 범위달라짐!
		
		<h3>1.page:현재페이지</h3>
		->현재 JSP 에서만 사용가능한 객체 (Servlet에서 사용불가)
		<h3>2.request==HttpServletRequest</h3>
		->요청받은 Servlet과 위임받은 페이지(JSP)에서 유지되는 객체
		
		<h3>3.session (입회,접속) *****</h3>
		->서버에 접속한 클라이언트를 나타내거나 관련 정보를 get/set할 수 있는 객체
			(session 객체는 서버에서 관리함)
			
		->session은 브라우저마다 하나씩 생성!!
		->사이트 접속~브라우저 종료 | 세션시간만료
		->session이 유지되는 상태에서는 아무곳에서나 session에 세팅된 값을 가져다가 사용
		(ex.로그인정보)
